...
> - ✅ ARP MOD 中の Delay/Reverb へのプチノイズ対策（§11）:
>   Note-Off のタイムスタンプ修正 / ボイス奪取の選択ロジック修正 / デクリッカー追加
...
## 11. ARP を MOD した時に Delay/Reverb へ入るプチノイズ（原因3件・修正済み）

### 原因A 🔴 Note-Off がブロック末尾に固定されていた（本命）

`Arpeggiator.cpp` のゲート減衰処理は Note-Off を常に `numSamples - 1`
（ブロック末尾）へ置いていました。一方 Note-On は `samplesProcessed`
（ブロック先頭寄り）に置かれます。`juce::MidiBuffer` はタイムスタンプ順に
読まれるため、同じブロック内で

```
Note-On (offset 0)  →  Note-Off (offset numSamples-1)
```

という順序になります。同じノート番号だと `SamplerEngine::handleMidi` の

```cpp
else if (msg.isNoteOff())
    for (auto& v : voices)
        if (v.isActive() && v.getMidiNote() == note) v.stopNote(true);
```

が走り、**鳴り始めたばかりのボイスが即リリースへ叩き落とされます**。

Gate を上げて `gateSamples` が `stepSamples` に近づくほど、また Rate / Gate に
MOD をかけて両者の関係が揺れるほど、この衝突が「ときたま」起きます。
生じた振幅の段差が Delay / Reverb のフィードバックに入り、
何度も繰り返されて目立つ、という症状になります。

**修正**: Note-Off を実際に切れるサンプル位置へ置く。あわせて
`samplesRemaining` をブロック先頭からの相対位置で持つようにし、
発音がブロック途中の場合のゲート長のブレも解消。

### 原因B 🟠 ボイス奪取が「一番新しい音」を切っていた

`PicoVoice::ageCounter` は各ボイスが**自分が何回発音したか**を数えているだけで、
グローバルな時刻ではありませんでした。`findOldestVoice()` はその最大値を返すので、
実際には「最も使い回されたボイス」＝多くの場合**直前に鳴り始めた音**を
奪っていました。ARP を速く回すとノート抜けとプチノイズが多発します。

**修正**: `SamplerEngine` が単調増加の通し番号を発行し、最小値＝最古とする。
さらに奪取の優先順位を「空き → リリース中で最古 → 最古」に変更。

### 原因C 🟠 ボイス再スタート時の段差そのもの

奪われたボイスは `startNote()` で `readPosition` と `envValue` が即座に
リセットされるため、直前の振幅から 0 へ1サンプルで飛びます。

**修正**: 直前の出力値を 3ms かけて raised cosine で 0 へ落とすテール
（デクリッカー）を加算。あわせて `stopNote(false)`（All Notes Off）も
即時 `reset()` から 3ms の高速リリースへ変更。

### 残っている既知の制約 🔵

`SamplerEngine::handleMidi` は MIDI イベントのサンプルオフセットを見ておらず、
1ブロック内のノートをすべてブロック先頭で発音します。
そのため ARP のタイミングはバッファサイズ単位に量子化されます
（512サンプル / 44.1kHz なら約11.6ms のジッタ）。
プチノイズの原因ではありませんが、高速な ARP のノリには影響します。
サブブロック分割レンダリングで解消できますが、`renderNextBlock` の
構造変更が必要なため今回は見送っています。

---

## 13. サブブロック処理への移行（LFO の「間」/ ARP のタイミング）

### 原因: ModMatrix がオーディオブロックに1回しか回っていなかった

`ModMatrix::processBlock()` は `processBlock` から1ブロックにつき1回しか呼ばれず、
LFO の**出力**が1周期あたりブロック数ぶんしか得られていなかった。
位相の進み方 (`inc = freq * numSamples / sampleRate`) 自体は正確で、
`phase` も `double` なのでドリフトもない。純粋に「間引き」の問題。

| バッファ | 更新レート | 1Hz | 5Hz | 10Hz | 20Hz | 30Hz |
|---|---|---|---|---|---|---|
| 512 | 86 Hz | 86点 | 17点 | 8.6点 | 4.3点 | 2.9点 |
| 1024 | 43 Hz | 43点 | 8.6点 | 4.3点 | 2.2点 | **1.4点** |
| 2048 | 22 Hz | 22点 | 4.3点 | 2.2点 | **1.1点** | **0.7点** |

1周期あたり10点を切ると、サンプリング点が周期に対してずれていく「うなり」が起きる。
たまたま2点続けて頂点付近に落ちるとそこで止まって見える／聴こえる。
これが「一周してから次に行くまでの間」の正体。

### 参照した Wavetable 側の実装

| 項目 | Wavetable | PicoSampler (修正前) |
|---|---|---|
| LFO の実行 | ボイスのサンプルループ内で毎サンプル | ブロックに1回 |
| 位相の型 | `double` | `double` ✅ 同じ |
| フィルター係数の更新 | 16サンプルごと + 線形補間 (`kCoefInterval`) | ブロックに1回 |
| Free モードの位相 | グローバルクロックで常時進行 | 元からプロセッサ側の単一 ModMatrix なので該当せず |
| 変調ソースの平滑化 | ノートオンで初期化 (Ver2.2b の主因) | 変調ソース側に平滑化なしのため該当せず |

Wavetable の Ver2.2b が直した4件のうち、PicoSampler に該当したのは
「実行レートが粗い」1点のみだった。

### 修正

`processBlock` を **16サンプル単位のサブブロック**に分割し、その単位で
ModMatrix → パラメータ構築 → 発音 → レンダリング → Filter → FX を回す。
Master セクションだけはループの外でブロック全体に一度掛ける。

- モジュレーション更新レート: 44.1kHz で **2756Hz**（30Hz の LFO でも 92点/周期）
- ARP のタイミング量子化: バッファ依存 (512 で約11.6ms) → **約0.36ms**
- 分割単位 16 は Wavetable の `kCoefInterval` に合わせた

### 副次的に直った既存バグ 🟠

`LadderFilter::setParameters()` の `driftAlpha` は
`1 - exp(-1/(0.01 * sampleRate))`、つまり**毎サンプル呼ばれる前提**の
10ms 時定数だった。実際にはブロックに1回しか呼ばれていなかったため、
実効時定数は

> 44.1kHz / 512サンプルバッファ → **約5.1秒**

Ladder LPF の Cutoff を回してから音が変わりきるまで5秒かかっていた計算になる。
16サンプル単位になったことで約0.16秒となり、Wavetable と同じ追従速度になった。

### 注意

サブブロック化によりパラメータ構築と ModMatrix の計算頻度が
バッファ512なら32倍になる。実測では問題ない見込みだが、
CPU 使用率は確認しておくこと。重い場合は `kControlBlockSize` を
32 や 64 へ上げれば線形に軽くなる（LFO の滑らかさとのトレードオフ）。
